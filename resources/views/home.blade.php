    ...
    <div class="w-full sm:px-6">
        ...
        <section class="flex flex-col my-16 break-words bg-white sm:border-1 sm:rounded-md sm:shadow-sm sm:shadow-lg">
            ...
            <form class="w-full p-6" method="POST" action="{{ route('url.store') }}">
                ...
                <div>
                    <div>
                        <input type="url" name="destination" id="destination" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="https://example.com" aria-describedby="email-description">
                    </div>
                    @if ($errors->any())
                        @error('destination')
                        ...
                        @enderror
                    @else
                      <div class="flex flex-col items-center justify-center mt-4 md:flex-row md:justify-between">
                          <p class="text-sm text-left text-gray-500" id="email-description">Enter the url that you want to shorten.</p>
                          <button type="submit" class="inline-flex items-center px-3 py-2 mt-4 text-sm font-medium leading-4 text-white bg-blue-900 border border-transparent rounded-md shadow-sm md:mt-0 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Shorten Url</button>
                      </div>
                    @endif
                </div>
            </form>
